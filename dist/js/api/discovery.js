import { API_BASE_URL, availableApiServers, currentApiServerIndex, statusDiv, setApiBaseUrl, setAvailableApiServers, setCurrentApiServerIndex } from '../state.js';
        ...
        request.onload = function () {
            if (request.status >= 200 && request.status < 300) {
                try {
                    ...
                    const serverUrls = items.map((x) => "https://" + x.name);
...
export async function initializeApiServer() {
    try {
        ...
        if (availableApiServers.length > 0 && currentApiServerIndex < availableApiServers.length) { // Ensure index is valid
            ...
            if (statusDiv)
                statusDiv.textContent = "API discovery failed (list error).";
...
async function useFallbackServers() {
    ...
    if (availableApiServers.length > 0 && currentApiServerIndex < availableApiServers.length) {
        ...
        if (statusDiv)
            statusDiv.textContent = "Using fallback API server."; // Update status
        return true;
    }
    console.error("Fallback server initialization failed.");
    if (statusDiv)
        statusDiv.textContent = "Error: All API options failed.";
    return false;
}
export function tryNextApiServer() {
    ...
    if (nextIndex >= availableApiServers.length) {
        ...
        if (statusDiv)
            statusDiv.textContent = "Error: All API servers failed in current list.";
    ...
    if (statusDiv)
        statusDiv.textContent = `Trying API: ${API_BASE_URL.split('//')[1].split('.')[0]}`; // Short name
    return true;
}

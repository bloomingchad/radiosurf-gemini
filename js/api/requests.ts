import {
    ...
    STATIONS_LIMIT,
    currentSearchOffset,
    stations, // existing stations list
    currentSearchModeDiv,
    setStations,
    Station, // Import the Station interface
    SearchType // Import SearchType
} from '../state.js';
...
// Type for suggestion API items (can be more specific if structure is known)
interface SuggestionItem {
    name: string;
    stationcount?: number; // Optional, present for tags, countries, languages
...
export async function fetchSuggestions(query: string): Promise<void> {
    ...
    const selectedSearchType: SearchType = searchTypeSelect.value as SearchType;
    ...
    const mapFn = (item: SuggestionItem | Station): string => (item.name ? item.name.toLowerCase() : '');
    ...
    try {
        ...
        const results: (SuggestionItem[] | Station[]) = await response.json(); 
...
export async function fetchStations(): Promise<void> {
    if (!API_BASE_URL) {
        ...
        if (!success) {
            ...
            if (statusDiv) statusDiv.textContent = "Error: API init failed.";
            return;
        }
    }
    if (statusDiv) statusDiv.textContent = 'Finding stations...';

    let STATIONS_URL: string;
    const searchActive = currentSearchType && currentSearchTerm;

    if (searchActive) {
        // Assertions because searchActive check implies they are not null
        const termToSearch = currentSearchTerm as string; 
        const typeToSearch = currentSearchType as Exclude<SearchType, null>;
    ...
    try {
        ...
        const fetchedStationsFromApi: Station[] = await response.json();
        const filteredStations = fetchedStationsFromApi.filter((s: Station) => s.url_resolved && (s.hls !== 1));
        ...
        if (statusDiv) statusDiv.textContent = 'API Error. Trying next...'; // Give user feedback
        if (tryNextApiServer()) {
            fetchStations(); 
        } else {

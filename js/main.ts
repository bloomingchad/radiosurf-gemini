...
import {
    ...
    sleepTimerId,
    debounceSuggestionTimer,
    SearchType, // Import the type alias
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    searchInput?.addEventListener('input', (event) => {
        ...
        if (query) {
            ...
            setDebounceSuggestionTimer(newTimer as unknown as number);
        } else {
            hideSuggestions();
        }
    });
    ...
    searchTypeSelect?.addEventListener('change', (event) => {
        ...
        const selectedType = target.value as SearchType;
        setCurrentSearchType(selectedType);

        if (searchInput) {
            let placeholderText = "e.g., rock";
            if (selectedType === 'country') placeholderText = "e.g., france";
            else if (selectedType === 'language') placeholderText = "e.g., spanish";
            else if (selectedType === 'name') placeholderText = "e.g., bbc radio";
    ...
    playerPool.forEach((playerElement, indexInPool) => {
        ...
        playerElement.addEventListener('pause', () => {
            if (indexInPool === activePlayerIndex) {
                setIsPlaying(false);
                if (playPauseBtn) playPauseBtn.innerHTML = '▶';
                if (sleepTimerId === null && statusDiv) statusDiv.textContent = 'Paused';
            }
        });
        ...
        playerElement.addEventListener('error', (e: Event) => {
            if (indexInPool === activePlayerIndex) {

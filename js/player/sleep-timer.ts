...
export function setSleepTimer(minutes: number): void {
    ...
    if (!isPlaying && stationListIndex === -1) {
        if (sleepTimerStatus) {
            ...
            setTimeout(() => { 
                if (sleepTimerId === null && sleepTimerStatus) sleepTimerStatus.textContent = ''; 
            }, 3000);
        }
        return;
    }
    ...
    setSleepTimerId(newTimerId as unknown as number); // Cast from NodeJS.Timeout to number for browser env
    
    const newIntervalId = setInterval(updateCountdown, 1000);
    setSleepTimerCountdownIntervalId(newIntervalId as unknown as number); // Cast as well
